...
class Urna(Transparencia):
    ...
    def to_csv(self):
        with open(f"{self.__zona}_{self.__secao}.csv", mode="w", newline='') as file:
            ...
            writer.writerow(['Seção', 'Zona', 'Título dos Eleitores'])

            for eleitores in self.__eleitores:
                writer.writerow([eleitores.secao, eleitores.zona, eleitores.get_titulo()])
...
if __name__ == '__main__':
    c1 = Candidato("Joao do Coco", "12312313-3", "12312313-3", 99)
    c2 = Candidato("Maria de Feira", "231231-3", "23434-2", 66)

    e1 = Eleitor("José da Silva", "2312303-3", "232323-3", 1, 54, 272)
    e2 = Eleitor("Mariana da Silva", "23123-4", "23232-3", 2, 54, 272)
    e3 = Eleitor("Catarina da Silva", "434435-3", "234532-3", 3, 54, 272)
